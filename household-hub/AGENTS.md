...
- **`?today` / `?tonight eat what?` / `?tonight dinner`** — **no today menu**. Leftover fridge check-in only (do not list today’s dishes). **Meals tab** shows leftover tonight + **tomorrow** for shop/prep. **Scheduled WhatsApp** at **10:00 HKT** (work days, reminder group) is a friendly check-in — look at the fridge and suggest dishes; she does not need to reply to the bot. Skip if leftover reminder is off, day off, or (for the ping) a leftover-empty window. Dish **names** (`?today honey wings`) still return a numbered similar-recipe list; reply `?1` / `?today 1, 4` to choose. Same category as an existing dish → confirm `?overwrite` (that category only) or `?also` (keep + add). Other categories are kept. YouTube / Instagram links still add into their category. `POST /api/meals/menu`. Seed file `data/dinner-overrides-seed.json` fills dates Admin has not saved yet (live Admin pick wins).
