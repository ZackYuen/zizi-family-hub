...
- **Zizi school prep** (K3 PM): checklist on `m4`–`f4` + leave-home `m6`–`f6` (`outingReminder`). Usual leave 12:30 (WhatsApp 11:30). **Interest class dates** (Term 1): Mon percussion / Tue Sumblox+magic / Fri fencing — leave **11:30** (WhatsApp 10:30); Tue pick-up **17:30**. Dates in `interestClasses` / `src/lib/interest-classes.ts`. Ask `? school prep` / `? 興趣班` / `? Sumblox`.
