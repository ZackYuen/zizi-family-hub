...
- **`?today` / `?tonight eat what?` / `?tonight dinner`** **show** the saved dinner (do not search or overwrite). **No daily random menu.** If nothing is saved, ask Charlene leftover fridge ingredients and suggest dishes; if no idea, ask us. **Scheduled WhatsApp** at **10:00 HKT** (work days, reminder group) is a friendly check-in only — look at the fridge and suggest dishes; she does not need to reply to the bot. Skip if a menu is already saved, it is her day off, or Admin → Settings leftover reminder is off. Dish **names** (`?today honey wings`) return a numbered similar-recipe list; reply `?1` / `?today 1, 4` to choose. Same category as an existing dish → confirm `?overwrite` (that category only) or `?also` (keep + add). Other categories are kept. YouTube / Instagram links still add into their category. `POST /api/meals/menu`. Seed file `data/dinner-overrides-seed.json` fills dates Admin has not saved yet (live Admin pick wins).
- **Zizi school prep** (K3 PM): 11:30 checklist on `m4`–`f4` + leave-home `m6`–`f6` (`outingReminder`). Usual leave 12:30 (WhatsApp 11:30). **Interest class dates** (Term 1): Mon percussion / Tue Sumblox+magic / Fri fencing — leave **11:30** (WhatsApp 10:30); Tue pick-up **17:30**. Dates in `interestClasses` / `src/lib/interest-classes.ts`. WhatsApp 1h before 12:30 includes FIL+EN steps (temperature, form, towels, masks, water, **empty food container for school tea time**, hair, pickup card; **Tue sportswear**). Ask `? school prep` / `? 體溫` / `? 接送卡` / `? 興趣班` / `? Sumblox`.
