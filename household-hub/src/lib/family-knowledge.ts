...
export function snapshotToKnowledgeText(snap: LiveFamilySnapshot): string {
  ...
  lines.push(
    "WhatsApp: ? replies go to Admin → Settings → group for bot replies (or bot GROUP_JIDS). 1-hour outing reminders and the 10:00 leftover-dinner ask go only to Admin → Settings → group for outing reminders — a different field; blank means no reminder is sent."
  );
  lines.push(
  ...
  if (snap.tomorrow) {
    lines.push("");
    const tomorrowDishesList = tonightDishes(snap.tomorrow);
    if (tomorrowDishesList.length) {
      lines.push(`Tomorrow's dinner (${snap.tomorrow.date}):`);
      for (const dish of tomorrowDishesList) {
        lines.push(
          `- ${dish.category}: ${getRecipeDisplayName(dish, "en")} / ${getRecipeDisplayName(dish, "fil")}`
        );
      }
    } else {
      lines.push(`Tomorrow's dinner (${snap.tomorrow.date}): none saved.`);
      lines.push(
        "Same leftover policy as tonight — no random menu."
      );
    }
  }
